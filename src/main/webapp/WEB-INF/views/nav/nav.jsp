...
<div class="modal fade" id="login-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form-login" method="POST" action="${root}/api/member/login">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success" id="find-pw" data-bs-toggle="modal"
                        data-bs-target="#find-pw-modal">비밀번호 찾기
                </button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form-mypage" method="POST" action="">
                    <div class="mb-3">
                        <label for="join-id" class="col-form-label">ID:</label>
                        <input type="text" class="form-control" id="mypage-id" name="mypage-id"
                               value="${userinfo.userId}" readonly/>
                    </div>
                    <div class="mb-3">
                        <label for="join-name" class="col-form-label">이름:</label>
                        <input type="text" class="form-control" id="mypage-name" name="mypage-name"
                               value="${userinfo.userName}" required/>
                    </div>
                    <div class="mb-3">
                        <label for="join-email" class="col-form-label">이메일:</label>
                        <input type="text" class="form-control" id="mypage-email" name="mypage-email"
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-warning ms-0" data-bs-toggle="modal"
                        data-bs-target="#pw-change-modal">비밀번호 변경
                </button>
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">닫기</button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form-pwchange" method="POST" action="">
                    ...
                    <input type="hidden" class="form-control" id="pw-change-new-id" name="pw-change-id"
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        let _savid = document.querySelector("#saveid");
        if ("${idck}" === "ok"){
            _savid.checked = true;
        } else{
            _savid.checked = false;
        }
    });
    ...
    if (loginCommitElement != null) {
        loginCommitElement.addEventListener("click", function (event) {
            event.preventDefault(); // 폼의 기본 제출 동작을 방지합니다.

            // 올바른 폼 ID로 수정
            let loginForm = document.getElementById("form-login");

            // 폼 데이터를 JSON 객체로 생성합니다.
            const loginData = {
                loginId: document.getElementById("loginId").value,
                loginPw: document.getElementById("loginPw").value,
                saveId: document.getElementById("saveid").checked // 체크박스의 체크 여부를 boolean 값으로 가져옵니다.
            };

            // fetch API를 사용하여 서버에 데이터를 전송합니다.
            fetch("${root}/api/member/login", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json' // 서버에 JSON 형식의 데이터를 보낼 것임을 명시합니다.
                },
                body: JSON.stringify(loginData), // 생성된 JSON 객체를 문자열로 변환하여 body에 포함시킵니다.
                credentials: 'include' // 쿠키를 포함시키기 위해 credentials 옵션 설정
            })
                .then(response => response.json())  // 서버로부터 받은 응답을 JSON으로 파싱합니다.
                .then(data => {
                    console.log(data); // 응답 데이터를 콘솔에 로깅합니다.

                        alert(data.data); // 실패 메시지를 사용자에게 보여줍니다.
                        window.location.href = '/'; // 로그인 성공 시 페이지를 이동합니다.

                })
                .catch(error => {
                    console.error('Error:', error); // 오류 발생 시 콘솔에 로깅합니다.
                    alert('Login failed'); // 사용자에게 로그인 실패를 알립니다.
                });
        });
    }
